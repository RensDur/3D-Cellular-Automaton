...
use crate::{AUTOMATON_SIZE, routes::gpu_get, K_MAX, appdata::dim3d::automata::{automaton_gpu_n_chemicals::CAChemicalGroup, automaton::CellularAutomaton3D}};

const AUTOMATON_SHADER_SRC: &str = include_str!("automaton_2d_gpu_shader.metal");
const ORDER_PARAM_SHADER_SRC: &str = include_str!("order_param_2d.metal");




#[derive(Clone, Serialize, Deserialize)]
pub struct GPUNChemicalsCellularAutomaton2D {
    pub grid: Vec<Vec<u8>>,
...
//
impl GPUNChemicalsCellularAutomaton2D {

    pub fn new(chemicals: Vec<CAChemicalGroup>) -> Self {
        GPUNChemicalsCellularAutomaton2D {
            grid: vec![vec![0u8; AUTOMATON_SIZE]; AUTOMATON_SIZE],
            chemicals,
            iteration_count: 0,
    ...
    pub fn export(&self) -> Vec<u8> {
        ...
        let mut res = vec![0u8; AUTOMATON_SIZE*AUTOMATON_SIZE];

        for x in 0..AUTOMATON_SIZE {
            for y in 0..AUTOMATON_SIZE {
                    res[x + y*AUTOMATON_SIZE] = self.grid[x][y];
            }
        }
    ...
    fn import(&mut self, data: Vec<u8>) {
        ...
        for x in 0..AUTOMATON_SIZE {
            for y in 0..AUTOMATON_SIZE {
                    self.grid[x][y] = data[x + y*AUTOMATON_SIZE];
            }
        }
    ...
    fn compute_order_parameter(&mut self) {
        autoreleasepool(|| {
            ...
            let sum = {
                let data: Vec<i8> = vec![0i8; AUTOMATON_SIZE*AUTOMATON_SIZE * (K_MAX+1)];
            ...
            let neighbours: Vec<i32> = vec![-1, 0,
                                             1, 0,
                                             0,-1,
                                             0, 1];
            ...
            let normalisation = 4.0 * (AUTOMATON_SIZE*AUTOMATON_SIZE) as f32;

            // Extract the obtained sums in the 'result_cell_sums' container
            let ptr = sum.contents() as *mut [i8; AUTOMATON_SIZE*AUTOMATON_SIZE * (K_MAX+1)];
            unsafe {
            ...
            for spec in 0..(self.chemicals.len()+1) {
                ...
                for i in 0..(AUTOMATON_SIZE*AUTOMATON_SIZE) {
...
//
impl CellularAutomaton3D for GPUNChemicalsCellularAutomaton2D {
    ...
    fn run_iteration(&mut self) {
        autoreleasepool(|| {
            ...
            let mut neighbours_promote: Vec<Vec<(i32, i32)>> = vec![];
            let mut neighbours_demote: Vec<Vec<(i32, i32)>> = vec![];
            ...
            let ptr = sum.contents() as *mut [u8; AUTOMATON_SIZE*AUTOMATON_SIZE];
            unsafe {
...
impl Source for GPUNChemicalsCellularAutomaton2D {
